...
import SupervisorApproveDocumentModal from "../Modals/actualLeaveSupervisorReviewRequestModal";
import SupervisorReviewedRequest from "../leaveFunctionalFiles/SupervisorReviewedRequest";
import {
    EllipsisOutlined,
    WarningTwoTone,
    DeleteOutlined,
...
const SupervisorPendingLeaveRequest = ({ token, role }) => {
    ...
    const initiateShowLeaveDetails = (leave) => {
        setLeaveDetails({
            ...
            actingPerson: leave.actualLeave.actingPerson.names, // Extract the name
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const responseExecutive = await axios.get(`${API}/leave/getsupervisors`, config);
            setManagers(responseExecutive.data)
            const response = await axios.get(`${API}/leave/actualleave/supervisor/getrequests/${empCode}`, config);
            ...
            if (response.data.dataPresent) {
                for (let i = 0; i < response.data.leave.length; i++) {
                    ...
                    if (currentLeaveInformation.actualLeave.supervisorApproval.requestStatus === "PENDING") {
                        count++
    ...
    const handleReview = async () => {
        ...
        try {
            await axios.put(`${API}/leave/actualleave/supervisor/review`, approveData, config)
    ...
    useEffect(() => {
        fetchData()
    }, [showApproveRequestModal, showLeaveDetails, openRecallModal])
    const filteredData = data.filter(searchedLeave => {
        let nestedValue;
        if (searchType.codeName === "empNames") {
        ...
        <>
            <div className="font-monospace mt-4" style={{ width: "110%" }}>
                ...
                <div className="card rounded-3 shadow-sm">
                                    ...
                                    {filteredData.map((pendingLeave, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{pendingLeave.actualLeave.actingPerson.names}</td>
                                                <td>
                                                    <UncontrolledDropdown>
                                                        <DropdownToggle
                                                            role="button"
                    ...
                    </div> : <SupervisorReviewedRequest openRecallModal={openRecallModal} setOpenRecallModal={setOpenRecallModal} leaveData={leaveData} token={token} />}
                </div>
                <div>
                    {showApproveRequestModal && <SupervisorApproveDocumentModal
...
export default SupervisorPendingLeaveRequest;
