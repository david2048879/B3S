...
import LineManagerReviewRequest from "../Modals/actualLeaveLineManagerReviewRequestModal";
import ManagerReviewedRequest from "../leaveFunctionalFiles/LineManagerReviewedRequest";
import {
    EllipsisOutlined,
    WarningTwoTone,
    DeleteOutlined,
...
const LineManagerPendingRequests = ({ token, role }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const hrStaffResponse = await axios.get(`${API}/leave/gethrstaff`, config);
            setHRStaff(hrStaffResponse.data)
            const response = await axios.get(`${API}/leave/actualleave/linemanager/getrequests/${empCode}`, config);
            ...
            if (response.data.dataPresent) {
                for (let i = 0; i < response.data.leave.length; i++) {
                    ...
                    if (currentLeaveInformation.actualLeave.lineManagerApproval.requestStatus === "PENDING") {
                        count++
    ...
    const handleReview = async () => {
        ...
        try {
            await axios.put(`${API}/leave/actualleave/linemanager/review`, approveData, config)
        ...
        <>
            <div className="font-monospace mt-5" style={{ width: "110%" }}>
                <div className="card rounded-3 shadow-sm">
                                        ...
                                        try {
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td>{pendingLeave.actualLeave.supervisorApproval.supervisor.names}</td>
                                                    <td>
                                                        <UncontrolledDropdown>
                                                            <DropdownToggle
                                                                role="button"
                    ...
                    </div> : <ManagerReviewedRequest leaveData={leaveData} />}
                </div>
                <div>
                    {showApproveRequestModal && <LineManagerReviewRequest
...
export default LineManagerPendingRequests;
