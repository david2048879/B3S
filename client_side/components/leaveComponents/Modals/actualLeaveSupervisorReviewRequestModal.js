...
const SupervisorApproveDocumentModal = ({ modalIsOpen, toggleModal, data, setData, handleReview, managers }) => {
    ...
    const [showChooseExecutive, setShowExecutive] = useState(false)
    ...
    useEffect(() => {
        if (confirmText === "Reject request") {
            ...
            if (confirmText === "Approve request") {
                if (data.lineManagerCode && data.lineManagerCode !== "") {
                    setDisableConfirmButton(false)
                }
                else {
                    setDisableConfirmButton(true)
        ...
        <Modal isOpen={modalIsOpen} toggle={() => toggleModal()} className="d-flex align-items-center justify-content-center" size='md'>
            <div>
                ...
                <div>
                    ...
                    {showChooseExecutive && <div className="mx-3">
                        <label htmlFor="itemName">Choose Line Manager</label>
                        <Select
                            onChange={(e) => setData({ ...data, lineManagerCode: e.value })}
                            options={managers.map((manager) => ({
                                value: manager.empCode,
                                label: manager.empNames
                            }))}
                        />
                    </div>}
                </div>
                <div className="row">
                    <div className="col">
                        {(showComment || showChooseExecutive) && <div className="m-4">
...
export default SupervisorApproveDocumentModal;