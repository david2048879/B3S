...
const DeleteDatesModal = ({ modalIsOpen,token, toggleModal, info,toggleParentModal,toggleMoreInfo }) => {
  const deletePlan = async (e) => {
    ...
    try {
      const response = await axios.put(`${API}/leave/planneddates/staff/delete`, info, config)
      ...
      <Modal isOpen={modalIsOpen} toggle={toggleModal} className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh' }}>
        <ModalBody>
          <div className="d-flex flex-column align-items-center">
            <p className="font-monospace status-message">Are you sure you want to delete this plan</p>
          </div>
        </ModalBody>
        <ModalFooter>
...
export default DeleteDatesModal;
