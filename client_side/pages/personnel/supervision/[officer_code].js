...
const Supervision = ({ token }) => {
	...
	const getStaff = async () => {
		const response = await axios.get(`${API}/profileOC/${officer_code}`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		});
		setCurrentStaff(response && response.data && response.data.myProfile);
		setStaffSupervision(
			response &&
				response.data &&
	...
	const getTeamMemberDetails = async (oC) => {
		const response = await axios.get(`${API}/profileEmpCode/${oC}`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		});
	...
	const removeSupervision = async (sup) => {
		...
		try {
			const response = await axios.post(
				`${API}/supervision/${currentStaff._id}`,
				{
					staffSupervision,
				},
				{
	...
	const handleSubmit = async (e) => {
		...
		if (supIndex !== -1) {
			...
			newSupervision.staffJobTitle =
				selectedMember.currentAppointment.jobTitle;
...
export default withHR(Supervision);
