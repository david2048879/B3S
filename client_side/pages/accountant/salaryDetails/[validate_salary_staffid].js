...
const salaryValidation = ({ token }) => {
	const router = useRouter();
	const { validate_salary_staffid } = router.query;
	...
	const getStaff = async () => {
		...
		const response = await axios.get(
			`${API}/profileAccountant/${validate_salary_staffid}`,
			{
				headers: {
...
export default withAccountant(salaryValidation);
