...
const destinationManagerComment = ({ token, user }) => {
	...
	const getTrip = async (tripid) => {
		...
		setState({
			...
			destinationManagerComment:
				response.data.tripRequest.destinationManagerComment,
		});
	...
	const handleSubmit = async (e) => {
		...
		if (
			!destinationManagerComment ||
			destinationManagerComment.length < 2
		) {
			setState({
				...state,
				error: `Please provide meaningfull comment on the field visit.`,
			});
			return;
		}
		setLoading(true);
		try {
			const response = await axios.put(
				`${API}/destinationManagerComment/${currentTrip._id}`,
				{
	...
	const destinationManagerCommentForm = () => (
		<Fragment>
			<div className="row">
			...
			<div className="row">
				<div className="form-group col-md-12">
					...
					<textarea
						rows="3"
						onChange={handleChange("destinationManagerComment")}
						className="form-control"
						value={destinationManagerComment}
						placeholder="Destination authority's comment"
					/>
				</div>
			</div>
			<hr />
...
export default withStaff(destinationManagerComment);
