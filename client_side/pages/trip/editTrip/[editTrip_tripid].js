...
const editTrip = ({ token }) => {
	const router = useRouter();
	const { editTrip_tripid } = router.query;
	...
	const getTrip = async () => {
		const response = await axios.get(
			`${API}/oneTripRequest/${editTrip_tripid}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
		...
		<Layout>
			<div className="col-md-10 offset-md-1 pt-2">
				...
				<div className="row alert alert-secondary">
					<div className="col-md-12">{editTripForm()}</div>
				</div>
			</div>
...
export default withStaff(editTrip);
